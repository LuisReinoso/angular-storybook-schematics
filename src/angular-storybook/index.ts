...
function createStory(options: ExtendedOptions): Rule {
  const lastPath = options.tagAsLabel
    ? `<${options.selector}>`
...
export default function(options: Schema): Rule {
  ...
  return (host: Tree) => {
    ...
    const replacePath: { from: string; to: string }[] = JSON.parse(
      options.replacePath
    );
    ...
    const extendedOptions: ExtendedOptions = {
      ...
      classifiedName,
      selector:
        options.selector || buildSelector(name, options.prefix, project.prefix)
    };
    return chain([
